...
\label{sec:hpo}
We tuned the gradient boosting models using Bayesian
optimization with Optuna~\cite{optuna}. For each model, 50 trials were
evaluated using 5-fold cross-validation with RMSE as the
objective. The full hyperparameter search spaces are reported
in Appendix~\ref{tab:search_space}, and the best-performing
configurations are summarized in Table~\ref{tab:tuned_params}.
