...
We employed \textbf{Bayesian optimization} via Optuna~\cite{optuna} to tune the gradient boosting models. Each model underwent 50 trials with 5-fold cross-validation per trial, optimizing for minimum RMSE.

\textbf{LightGBM Search Space:}
\begin{itemize}
    \item \texttt{num\_leaves}: [20, 100]
    \item \texttt{learning\_rate}: [0.01, 0.1] (log-scale)
    \item \texttt{feature\_fraction}: [0.6, 1.0]
    \item \texttt{bagging\_fraction}: [0.6, 1.0]
    \item \texttt{min\_child\_samples}: [5, 50]
    \item \texttt{reg\_alpha}, \texttt{reg\_lambda}: [0.001, 10.0] (log-scale)
\end{itemize}

\textbf{XGBoost Search Space:}
\begin{itemize}
...
\textbf{CatBoost Search Space:}
\begin{itemize}
    \item \texttt{depth}: [4, 10]
    \item \texttt{learning\_rate}: [0.01, 0.1] (log-scale)
    \item \texttt{l2\_leaf\_reg}: [1.0, 10.0]
    \item \texttt{bagging\_temperature}: [0.0, 1.0]
\end{itemize}

Table~\ref{tab:tuned_params} presents the optimal hyperparameters discovered through this process.
